...
for i = 1:m
    ...
    h_thetax = zeros(num_labels, 1);
    y_i(y(i)) = 1;
    a_1 = (X(i, :))';
    z_2 = Theta1 * a_1;
    a_2 = [1;sigmoid(z_2)];
    z_3 = Theta2 * a_2;
    h_thetax = sigmoid(z_3);
    delta_3 = h_thetax - y_i;
    delta_2 = Theta2' * delta_3 .* sigmoidGradient([1;z_2]);
    delta_2 = delta_2(2:end);
    Theta1_grad = Theta1_grad + delta_2 * a_1';
    Theta2_grad = Theta2_grad + delta_3 * a_2';
    %h_thetax = sigmoid(Theta2 * [1;sigmoid(Theta1 * (X(i, :))')]);
    J = J + sum(-y_i .* log(h_thetax) - (1 - y_i) .* log(1 - h_thetax));
end
J = J / m;
